CREATE PROCEDURE [dbo].[SetSuggestions]
( 
	@Treatment INT, 
	@Group INT,
	@Period INT 
...
BEGIN TRY		
	...
	BEGIN
		INSERT INTO Shares
			SELECT Owner , Treatment , Group# , [Period] , @i , Volume , 0 , 0
            FROM Shares
            WHERE Treatment = @Treatment AND [Group#] = @Group AND [Period] = @Period AND Choice = 0;
...
BEGIN CATCH
	...
	INSERT INTO ErrorLog VALUES (GETDATE(),ERROR_NUMBER(), ERROR_MESSAGE(), 30);
	--PRINT  CAST(ERROR_NUMBER() AS VARCHAR) + ' Error in SetSuggestions: ' + ERROR_MESSAGE();
	RETURN ERROR_NUMBER();
END CATCH;	
COMMIT TRANSACTION;
PRINT 'Completed';
